...
const room = {
    ...
    votingRound: 0,
    sessionPhase: 'open',
    waitingQueue: [] // Array of { ws, joinData } for users waiting to join
};

// Lock for join operations
...
// Process waiting queue - try to add waiting users when spots open
async function processWaitingQueue() {
    ...
    joinLock = joinLock.then(async () => {
        ...
        while (activeCount < MAX_PARTICIPANTS && room.waitingQueue.length > 0) {
            const { ws, joinData } = room.waitingQueue.shift();
            ...
            if (!participantNumber) {
                // Room became full, put back in queue
                room.waitingQueue.unshift({ ws, joinData });
                break;
            }
...
async function handleJoin(ws, data) {
    ...
    joinLock = joinLock.then(async () => {
        ...
        const activeCount = cleanupDeadConnections();
        
        // If room is full, add to waiting queue
        if (activeCount >= MAX_PARTICIPANTS) {
            console.log(`Room is full (${activeCount}/${MAX_PARTICIPANTS}). Adding to waiting queue.`);
            room.waitingQueue.push({ ws, joinData: data });
            ws.send(JSON.stringify({
                type: 'waiting',
                message: `Room is full. You are #${room.waitingQueue.length} in the queue. You will be notified when a spot opens.`,
                position: room.waitingQueue.length
            }));
            return null;
        }
        ...
        const participantNumber = getAvailableNumber();
        if (!participantNumber) {
            // Shouldn't happen if cleanup worked, but just in case
            room.waitingQueue.push({ ws, joinData: data });
            ws.send(JSON.stringify({
                type: 'waiting',
                message: `Room is full. You are #${room.waitingQueue.length} in the queue.`,
                position: room.waitingQueue.length
            }));
            return null;
        }
...
function removeParticipant(participantId) {
    ...
    // Process waiting queue when someone leaves
    processWaitingQueue();
}
...
wss.on('connection', (ws) => {
    ...
    ws.on('close', () => {
        ...
        if (participantId && room.participants.has(participantId)) {
            ...
            const queueIndex = room.waitingQueue.findIndex(item => item.ws === ws);
            if (queueIndex !== -1) {
...
server.listen(PORT, () => {
    ...
    console.log(`Single room with max ${MAX_PARTICIPANTS} participants`);
    console.log(`Waiting queue enabled for overflow users`);
});
