        ...
        try {
            ...
            switch (data.type) {
                case 'join':
                    ...
                    // STRICT: Check room capacity BEFORE adding - must be less than 20 (not <= 20)
                    // This ensures exactly 20 maximum (0-19 before adding = 20 total)
                    const currentCount = room.participants.size;
                    if (currentCount >= 20) {
                        console.log(`Room ${roomId} is full (${currentCount}/20). Rejecting new participant.`);
                    ...
                    // Assign number - this should never fail if we're checking correctly above
                    participantNumber = assignNumber(room);
                    if (!participantNumber) {
                        console.log(`No number available in room ${roomId} (${currentCount} participants). Rejecting.`);
                        ws.send(JSON.stringify({
                            type: 'error',
                            message: 'Room is full (max 20 participants)'
                        }));
                    ...
                    // Final check before adding - ensure we're not exceeding 20
                    if (room.participants.size >= 20) {
                        console.log(`Race condition detected: room ${roomId} became full during join. Rejecting.`);
                        ws.send(JSON.stringify({
                            type: 'error',
                            message: 'Room is full (max 20 participants)'
                        }));
                    ...
                    // Add participant - this should keep us at or below 20
                    ...
                    // Verify we didn't exceed 20 (safety check)
                    if (room.participants.size > 20) {
                        console.error(`ERROR: Room ${roomId} exceeded 20 participants (${room.participants.size})! Removing last added.`);
