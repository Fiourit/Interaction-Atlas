...
const roomJoinQueues = new Map(); // roomId -> Promise queue for serializing joins

// Process joins one at a time per room to prevent race conditions
async function processJoinQueue(roomId, joinOperation) {
    if (!roomJoinQueues.has(roomId)) {
        roomJoinQueues.set(roomId, Promise.resolve());
    }
    
    const queue = roomJoinQueues.get(roomId);
    // Chain the operation and ensure errors don't break the queue
    const newPromise = queue.then(async () => {
        try {
            return await joinOperation();
        } catch (err) {
            console.error('Error in join operation:', err);
            // Don't throw - allow queue to continue even if one join fails
            return null;
        }
    });
    
    roomJoinQueues.set(roomId, newPromise);
    return newPromise;
}

function assignNumber(room) {
    // Assign numbers 1-20 to participants
    const usedNumbers = Array.from(room.participants.values())
        ...
        try {
            ...
            switch (data.type) {
                case 'join':
                    ...
                    console.log(`Join request for room: "${roomId}" (received: "${data.roomId || 'undefined'}")`);
                    const tempParticipantId = `${Date.now()}-${Math.random()}`;

                    // Process join through queue to prevent race conditions
                    processJoinQueue(roomId, async () => {
                        // Ensure room exists
                        if (!rooms.has(roomId)) {
                            rooms.set(roomId, {
                        ...
                        // Clean up dead connections first (check AND remove in one pass)
                        let activeCount = 0;
                        ...
                        // ATOMIC CHECK: Count active participants ONLY
                        if (activeCount >= 20) {
                            console.log(`Room ${roomId} is full (${activeCount}/20 active participants). Rejecting new participant.`);
                        ...
                        // Assign number based on active participants only
                        const tempParticipantNumber = assignNumber(room);
                        if (!tempParticipantNumber) {
                            console.log(`No number available in room ${roomId} (${activeCount} active participants). Rejecting.`);
                            if (ws.readyState === WebSocket.OPEN) {
                                ws.send(JSON.stringify({
                                    type: 'error',
                                    message: 'Room is full (max 20 participants)'
                        ...
                        // Final atomic check before adding
                        ...
                        if (finalActiveCount >= 20) {
                            console.log(`Room ${roomId} became full during join processing. Rejecting.`);
                            if (ws.readyState === WebSocket.OPEN) {
                                ws.send(JSON.stringify({
                                    type: 'error',
                                    message: 'Room is full (max 20 participants)'
                        ...
                        // Now set the actual participantId and number
                        participantId = tempParticipantId;
                        participantNumber = tempParticipantNumber;

                        // Add participant - this is now atomic within the queue
                        ...
                        if (verifyActiveCount > 20) {
                            console.error(`CRITICAL ERROR: Room ${roomId} exceeded 20 participants (${verifyActiveCount})! Removing last added.`);
                ...
                if (room.participants.size === 0) {
                    setTimeout(() => {
                        if (rooms.has(roomId) && rooms.get(roomId).participants.size === 0) {
                            rooms.delete(roomId);
                            // Clean up join queue for this room
